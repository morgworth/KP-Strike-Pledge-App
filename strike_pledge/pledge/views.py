        ...
        if form.is_valid():
            ...
            if email_hash == hashlib.sha1(work_email.lower().encode()).hexdigest():
                ...
                if tweet != '':
                    ...
                    try:
                        api.PostUpdate(tweet[0:237] + ' #kaiserstrike #wearekaiserworkers @aboutKP')
                    except:
                        print('')
                try:
...
def helpView(request):
    if request.method == 'GET':
        ...
        if form.is_valid():
            email = EmailMessage('Question from Contact page', form.cleaned_data['question'], 'noreply@kaiserstrike.org', ['inquiries@kaiserstrike.org'], reply_to=[form.cleaned_data['home_email']])
            try:
